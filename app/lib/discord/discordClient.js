...
class DiscordClient {
  ...
  request = (options = {}) => {
    const schema = yup.object().shape({
      ...
      token: yup.string().default(APP_DISCORD_BOT_TOKEN),
      type: yup.string(),
    });
  ...
  getGuildMember = async userId => this.request({
    type: 'getGuildMember',
  }).get(`/guilds/${APP_DISCORD_GUILD_ID}/members/${userId}`)
  ...
  listGuildMembers = (limit = 1, after = 0) => {
    ...
    }).get(`/guilds/${APP_DISCORD_GUILD_ID}/members`, {params})
      .then(response => response.data);
  };

  updateRoles = async (userId, newRoles) => this.request({
    type: 'updateRoles',
  }).patch(`/guilds/${APP_DISCORD_GUILD_ID}/members/${userId}`, {
      roles: newRoles,
  ...
  addGuildMember = async (accessToken, userId) => this.request()
    .put(`/guilds/${APP_DISCORD_GUILD_ID}/members/${userId}`, {
      access_token: accessToken,
    }).then(response => response.data);

  getGuildRoles = async () => this.request()
    .get(`/guilds/${APP_DISCORD_GUILD_ID}/roles`)
    .then(response => response.data)
  ...
  addRoleToUser = async (userId, roleId) => this.request({
    type: 'addRoleToUser',
  }).put(`/guilds/${APP_DISCORD_GUILD_ID}/members/${userId}/roles/${roleId}`)
    .then(response => response.data);
